...
async fn update_coin_state(
    ...
    sqlx::query!(
        "
        UPDATE coins
        SET created_height = ?, spent_height = ?, transaction_hash = ?
        WHERE hash = ?
        ",
        created_height,
        spent_height,
...
async fn sync_coin(
    ...
    sqlx::query!(
        "
        UPDATE coins SET is_synced = 1, hint = ?, kind = ? WHERE hash = ?
        ",
        hint,
        kind,
        coin_id
...
async fn unsync_coin(conn: impl SqliteExecutor<'_>, coin_id: Bytes32) -> Result<()> {
    ...
    sqlx::query!("UPDATE coins SET is_synced = 0 WHERE hash = ?", coin_id)
        .execute(conn)
        .await?;
...
async fn is_coin_locked(conn: impl SqliteExecutor<'_>, coin_id: Bytes32) -> Result<bool> {
    ...
    let row = sqlx::query!(
        ...
        AND (offers.hash IS NULL OR offers.status > 0)
...
async fn get_transaction_coins(
    ...
    query.push("
        SELECT 
            ...
            cs.kind, 
            amount,
            COALESCE (cat_coins.p2_puzzle_hash, nft_coins.p2_puzzle_hash, did_coins.p2_puzzle_hash, puzzle_hash) AS p2_puzzle_hash,
            ...
            (SELECT COUNT(*)
                FROM derivations d 
                WHERE d.p2_puzzle_hash = COALESCE(cat_coins.p2_puzzle_hash, nft_coins.p2_puzzle_hash, did_coins.p2_puzzle_hash, cs.puzzle_hash)) AS derivation_count
        FROM coin_states cs
