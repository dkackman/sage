...
pub async fn storage_open_database(
    ...
    validate_name(&req.name, "database name")
        .map_err(|err| std::io::Error::other(err.to_string()))?;

    let app = read_installed_app_by_id(&base_path, &app_id)
        .map_err(|err| std::io::Error::other(format!("failed to read app {}: {err}", app_id)))?;

    let mut conn = open_connection(&app, &req.name)
        .await
        .map_err(|err| std::io::Error::other(format!("failed to open database: {err}")))?;

    let current = get_current_version(&mut conn)
        .await
        .map_err(|err| std::io::Error::other(format!("failed to read version: {err}")))?;
    ...
    if req.version > current {
        set_current_version(&mut conn, req.version)
            .await
            .map_err(|err| std::io::Error::other(format!("failed to set version: {err}")))?;
    }
...
pub async fn storage_delete_database(
    ...
    if path.exists() {
        fs::remove_file(&path).map_err(|err| {
            std::io::Error::other(format!("failed to remove {}: {err}", path.display()))
        })?;
    }
...
pub async fn storage_describe_database(
    ...
    let mut conn = open_connection(&app, &db_name)
        .await
        .map_err(|err| std::io::Error::other(format!("failed to open database: {err}")))?;
    ...
    let store_rows = sqlx::query("SELECT store_name FROM sage_object_stores ORDER BY store_name ASC")
        .fetch_all(&mut conn)
        .await
        .map_err(|err| std::io::Error::other(format!("failed to query stores: {err}")))?;

    let stores = store_rows
        .into_iter()
        .map(|row| SageStorageObjectStoreInfo {
    ...
    let index_rows = sqlx::query(
        ...
        .fetch_all(&mut conn)
        .await
        .map_err(|err| std::io::Error::other(format!("failed to query indexes: {err}")))?;

    let indexes = index_rows
        .into_iter()
        .map(|row| SageStorageIndexInfo {
            store: row.try_get::<String, _>(0).unwrap_or_default(),
...
pub async fn storage_create_object_store(
    ...
    validate_name(&req.db_name, "database name")
        .map_err(|err| std::io::Error::other(err.to_string()))?;
    validate_name(&req.store_name, "store name")
        .map_err(|err| std::io::Error::other(err.to_string()))?;

    let app = read_installed_app_by_id(&base_path, &app_id)
        .map_err(|err| std::io::Error::other(format!("failed to read app {}: {err}", app_id)))?;

    let mut conn = open_connection(&app, &req.db_name)
        .await
        .map_err(|err| std::io::Error::other(format!("failed to open database: {err}")))?;
    ...
    sqlx::query("INSERT OR IGNORE INTO sage_object_stores (store_name) VALUES (?1)")
        ...
        .execute(&mut conn)
        .await
        .map_err(|err| std::io::Error::other(format!("failed to create store: {err}")))?;
...
pub async fn storage_create_index(
    ...
    validate_name(&req.store_name, "store name")
        .map_err(|err| std::io::Error::other(err.to_string()))?;
    validate_name(&req.index_name, "index name")
        .map_err(|err| std::io::Error::other(err.to_string()))?;
    ...
    ensure_store_exists(&mut conn, &req.store_name)
        .await
        .map_err(|err| std::io::Error::other(err.to_string()))?;
    ...
    sqlx::query("INSERT OR IGNORE INTO sage_indexes (store_name, index_name) VALUES (?1, ?2)")
        ...
        .execute(&mut conn)
        .await
        .map_err(|err| std::io::Error::other(format!("failed to create index: {err}")))?;
...
pub async fn storage_get(
    ...
    let row = sqlx::query(
        ...
        .await
        .map_err(|err| std::io::Error::other(format!("failed to read value: {err}")))?;

    let value = row
        .map(|row| row.try_get::<Vec<u8>, _>(0))
        .transpose()
        .map_err(|err| std::io::Error::other(format!("failed to decode value: {err}")))?;
...
pub async fn storage_put(
    ...
    let mut tx = conn
        .begin()
        .await
        .map_err(|err| std::io::Error::other(format!("failed to start tx: {err}")))?;
    ...
    sqlx::query(
        ...
        .execute(&mut *tx)
        .await
        .map_err(|err| std::io::Error::other(format!("failed to write record: {err}")))?;
    ...
    sqlx::query("DELETE FROM sage_index_entries WHERE store_name = ?1 AND primary_key = ?2")
        ...
        .execute(&mut *tx)
        .await
        .map_err(|err| std::io::Error::other(format!("failed to clear old index entries: {err}")))?;

    for index_value in &req.index_values {
        validate_name(&index_value.index_name, "index name")
            .map_err(|err| std::io::Error::other(err.to_string()))?;
        ...
        sqlx::query(
            ...
            .execute(&mut *tx)
            .await
            .map_err(|err| std::io::Error::other(format!("failed to write index entry: {err}")))?;
    }

    tx.commit()
        .await
        .map_err(|err| std::io::Error::other(format!("failed to commit tx: {err}")))?;
...
pub async fn storage_delete(
    ...
    sqlx::query("DELETE FROM sage_records WHERE store_name = ?1 AND primary_key = ?2")
        ...
        .execute(&mut conn)
        .await
        .map_err(|err| std::io::Error::other(format!("failed to delete record: {err}")))?;
...
pub async fn storage_clear(
    ...
    sqlx::query("DELETE FROM sage_records WHERE store_name = ?1")
        ...
        .execute(&mut conn)
        .await
        .map_err(|err| std::io::Error::other(format!("failed to clear store: {err}")))?;
...
pub async fn storage_count(
    ...
    let count = sqlx::query_scalar::<_, i64>(
        ...
        .fetch_one(&mut conn)
        .await
        .map_err(|err| std::io::Error::other(format!("failed to count records: {err}")))?;
...
pub async fn storage_get_all(
    ...
    let rows = sqlx::query(
        ...
        .fetch_all(&mut conn)
        .await
        .map_err(|err| std::io::Error::other(format!("failed to query records: {err}")))?;

    let out = rows
        .into_iter()
        .map(|row| -> std::io::Result<SageStorageValueRecord> {
            let key: Vec<u8> = row
                .try_get(0)
                .map_err(|err| std::io::Error::other(err.to_string()))?;
...
pub async fn storage_get_all_from_index(
    ...
    let rows = sqlx::query(
        ...
        .fetch_all(&mut conn)
        .await
        .map_err(|err| std::io::Error::other(format!("failed to query index: {err}")))?;
